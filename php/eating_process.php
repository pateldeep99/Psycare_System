...
<?php 
 ...
 if($_GET){
	//We need total question in process file too
 	$query = "SELECT * FROM eating_question";
	...
	if ($number === 1) {
 	 ...
 	 if($number == $total_questions)
 	 {
		...
		$str="INSERT into history(name,type,score) VALUES('".$_SESSION['name']."','eating' ,'".$_SESSION['score']." %')";
		$connection->query($str);	
 	  	header("LOCATION: eating_score.php");
 	 }
 	 else{
 	 	header("LOCATION: eating_quiz.php?n=". $next);
 	 }
 }
?>