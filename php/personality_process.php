...
<?php 
 ...
 if($_GET){
	//We need total question in process file too
 	$query = "SELECT * FROM depression_question";
	...
	if ($number === 1) {
 	 ...
 	 if($number == $total_questions)
 	 {
		...
		$str="INSERT into history(name,type,score) VALUES('".$_SESSION['name']."','personality' ,'".$_SESSION['score']." %')";
		$connection->query($str);	
 	  	 	header("LOCATION: personality_score.php");
 	 }
 	 else{
 	 	header("LOCATION: personality_quiz.php?n=". $next);
 	 }
 }
?>