...
<?php 
 ...
 if($_GET){
	//We need total question in process file too
 	$query = "SELECT * FROM bipolar_question";
	...
	if ($number === 1) {
 	 ...
 	 if($number == $total_questions)
 	 {
		...
		$str="INSERT into history(name,type,score) VALUES('".$_SESSION['name']."','bipolar' ,'".$_SESSION['score']." %')";
		$connection->query($str);	
 	  	header("LOCATION: bipolar_score.php");
 	 }
 	 else{
 	 	header("LOCATION: bipolar_quiz.php?n=". $next);
 	 }
 }
?>